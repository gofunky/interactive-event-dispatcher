...
{{ if (not ($input.description | strings.Contains "<--")) }}
...
![Default](https://img.shields.io/badge/Default-{{ if (has $input "default") }}{{ if (ne $input.default "") }}{{ tmpl.Exec "sanitize_url" $input.default }}{{ else }}'_'{{ end }}{{ else }}none{{ end }}-{{ if (has $input "default") }}{{ if (ne $input.default "") }}{{ strings.Trunc 6 (crypto.SHA1 $input.default) }}{{ else }}inactive{{ end }}{{ else }}inactive{{ end }}?style=flat-square)
